...
| OD-02 | **Free-plan caps.** How many active products and favourite counts Free allows. | 100 active products, 1 favourite count. Existing data is never locked. A cap only blocks creating a new product or favourite. | `src/modules/billing/limits.ts` (`FREE_CAPS`) |
