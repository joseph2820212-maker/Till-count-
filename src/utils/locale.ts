...
// PHONE's locale.
/** Units & formats → Number format. 'auto' follows the language. */
export type NumberFormatOverride = 'auto' | 'comma-dot' | 'dot-comma' | 'space-comma';
let numberFormatOverride: NumberFormatOverride = 'auto';
export function setNumberFormatOverride(f: NumberFormatOverride): void { numberFormatOverride = f; }

function separators(language: string): { thousands: string; decimal: string } {
  if (numberFormatOverride === 'comma-dot') return { thousands: ',', decimal: '.' };
  if (numberFormatOverride === 'dot-comma') return { thousands: '.', decimal: ',' };
  if (numberFormatOverride === 'space-comma') return { thousands: '\u202F', decimal: ',' };
  const commaDecimal = language === 'tr' || language === 'fr' || language === 'es' || language === 'de';
  return { thousands: language === 'fr' ? '\u202F' : commaDecimal ? '.' : ',', decimal: commaDecimal ? ',' : '.' };
}
...
export function groupNumber(
  ...
  const { thousands, decimal } = separators(language);
  const neg = v < 0 ? '-' : '';
  ...
  if (numberFormatOverride !== 'auto' || language !== 'es' || int.length > 4) {
