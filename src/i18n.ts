...
// Non-English files are machine-translated for review
// and must be human-proofread before the store release.
...
const LANGUAGE_KEY = 'tillcount:device:language';
const LANGUAGE_TRANSITION_KEY = 'tillcount:device:languageTransition';
...
async function reloadApp(): Promise<boolean> {
  ...
  try {
    const result = await Promise.race<boolean>([
      reloadAppAsync('TillCount language direction changed').then(() => true),
